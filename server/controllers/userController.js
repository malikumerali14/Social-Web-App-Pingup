...
export const updateUserData = async (req, res) => {
    try {
        ...
        if (profile) {
            const response = await cloudinary.uploader.upload(profile.path);
        ...
        if (cover) {
            const response = await cloudinary.uploader.upload(cover.path);
            const url = response.url;
...
export const sendConnectionRequest = async (req, res) => {
    try {
        ...
        if (connectionRequests.length >= 20) {
            return res.json({ success: false, message: "You have sent more than 20 connection requests in the past 24 hours" })
        }
...
export const getUserConnections = async (req, res) => {
    try {
        const { userId } = req.auth();
        const user = await User.find({ userId }).populate('connections following followers');
